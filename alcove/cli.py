...
def _dispatch_search(query, k=3, mode="semantic", collections=None):
...
def cmd_search(args):
    ...
    collections = getattr(args, "collection", None) or None
    result = _dispatch_search(args.query, k=args.k, mode=mode, collections=collections)
    if args.json:
...
def _add_search_parser(sub, name, hidden=False):
    ...
    p.add_argument(
        ...
        action="append",
        default=[],
        help="Limit search to a named collection; repeat for multiple collections",
    )
    p.add_argument(
...
def main():
    ...
    p_mirrulations = sub.add_parser(
        "mirrulations-demo",
        help="Ingest a local Mirrulations text subset into a named collection",
    )
    ...
    p_mirrulations.add_argument(
        ...
        help="Named collection to tag the indexed records with (default: mirrulations_docs)",
    )
